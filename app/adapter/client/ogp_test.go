...
func TestOGPFetcher_Fetch(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			want: domain.OGPData{Title: "OG Title", Description: "OG Description", Image: "https://example.com/og.png"},
		},
		{
		...
		},
		{
			...
			want: domain.OGPData{Description: "OG Description"},
		},
		{
			...
			want: domain.OGPData{Description: "Fallback Description"},
		},
		{
...
func TestOGPFetcher_Fetch_BlocksLoopback(t *testing.T) {
	...
	f := (&Client{}).NewOGPFetcher() // Dialer.Control が 127.0.0.1 を弾くはず
	got, err := f.Fetch(context.Background(), srv.URL)
	require.Error(t, err)
	assert.Equal(t, domain.OGPData{}, got)
}
